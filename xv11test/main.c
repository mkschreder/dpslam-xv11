...
{	
	...
	for(i = 0; i < frames_number; i++){
		for(j=0;j<4;++j){
			int distance = frames[i].readings[j].distance; 
			int index = (frames[i].index-0xA0) * 4 + j; 

			if(frames[i].readings[j].invalid_data==0 && index < 180){
				fprintf(f, "%f ", (float)distance / 1000.0); 
			}
			else {
				fprintf(f, "2 "); 
				//printf("%d 0\n", index);
	...
	fprintf(f, "\n"); 
	fclose(f); 
	//printf("VEL [ %f %f ]\n", avg_speed_x, avg_speed_y); 
	//printf("CLOSEST [ %d %d ], FURTHEST [ %d %d ], AVG[ %f %f ]\n", smallest_ang, smallest_dist, largest_angle, largest_dist, average_x, average_y); 
	/*
	for(i=0;i<360;++i)
	{
		printf("%d",i);
